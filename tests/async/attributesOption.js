buster.testCase("Backbone.Validation.Async Setting options.attributes", {
    setUp: function () {
    ...
    "to an string array": {
        ...
        "only the attributes in array should be validated": function (done) {
        ...
        "when all the attributes in array are valid": {
            ...
            "validation will pass": function (done) {
                this.model.validate(null, null, done(function valid() {
                    assert(true);
    ...
    "to an function returning an string array": {
        ...
        "only the attributes returned by the function should be validated": function (done) {
            this.model.validate(null, null, done(function valid() {
                assert(false);
            }), done(function invalid(errors) {
                assert.defined(errors.name);
    ...
    "to 'inputNames' builtin attributeLoader": {
        ...
        "only the attributes with present in form should be validated": function (done) {
            this.model.validate(null, null, done(function valid() {
                assert(false);
            }), done(function invalid(errors) {
                assert.defined(errors.name);
        ...
        "submit and buttons should not be included in attribute array": function () {
            var attrs = Backbone.Validation.Async.attributeLoaders.inputNames(this.view);
    ...
    "to an custom attributeLoader": {
        setUp: function () {
            _.extend(Backbone.Validation.Async.attributeLoaders, {
        ...
        "only the attributes returned by the registered function should be validated": function (done) {
            this.model.validate(null, null, done(function valid() {
                assert(false);
            }), done(function invalid(errors) {
                assert.defined(errors.age);
