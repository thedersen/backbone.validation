buster.testCase('Backbone.Validation.Async Binding to view with model', {
        setUp: function() {
...
buster.testCase('Binding to view with optional model', {
        ...
        "and unbinding":{
                setUp: function(){
                        Backbone.Validation.Async.unbind(this.view, {model: this.model});
                },
...
buster.testCase('Binding to view with optional collection', {
        setUp: function() {
                ...
                var View = Backbone.View.extend({
                        render: function() {
                                Backbone.Validation.Async.bind(this, {collection: self.collection});
                        }
                });
                this.view = new View();
        ...
        "unbinds all models in collection when unbinding view": function() {
                Backbone.Validation.Async.unbind(this.view, {collection: this.collection});
...
buster.testCase('Binding multiple views to same model', {
    ...
    "each view can be unbind separately from the same model": {
        setUp: function() {
            this.model.set('name', '');
            this.view2.render();
            Backbone.Validation.Async.unbind(this.view2);
        },

        "first element is invalid and has class invalid": function(done) {
            var self = this;
            var always = function always() {
                self.model.isValid('name', done(function valid() {
                    assert(false);
                }), done(function invalid() {
                    assert(true);
                    assert(self.name.hasClass('invalid'));
                }));
            };
            this.model.validate(null, null, always, always);
        },

        "second element is invalid and has not class invalid": function(done) {
            var self = this;
            var always = function always() {
                self.model.isValid('surname', done(function valid() {
                    assert(false);
                }), done(function invalid() {
                    assert(true);
                    refute(self.surname.hasClass('invalid'));
                }));
            };
            this.model.validate(null, null, always, always);
        }
